...
import { selectErrorCount } from './selectors'
import { errorCountSlice } from './errorCountSlice'
...
export const incrementIfOddAsync =
  (amount: number): ReduxThunkAction =>
    (dispatch, getState) => {
      const currentValue = selectErrorCount(getState())
